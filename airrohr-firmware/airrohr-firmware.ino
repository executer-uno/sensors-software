...
/*****************************************************************/
// increment on change
#define SOFTWARE_VERSION "NRZ-2017-100-B4"
...
/*****************************************************************/
String check_display_value(float value, float undef, int len, int str_len) {
	String s = (value != undef ? Float2String(value,len) : "-");
	while (s.length() < str_len) { s = " " + s; }
	return s;
}
...
String Float2String(const float value, unsigned int digits) {
...
void webserver_values() {
    if (WiFi.status() != WL_CONNECTED) {
        ...
        page_content += empty_row;
        page_content += table_row_from_value(***REMOVED***, FPSTR(INTL_SIGNAL),  String(signal_strength), "dBm");
        page_content += table_row_from_value(***REMOVED***, FPSTR(INTL_QUALITAT), String(signal_quality), "%");
        page_content += F("</table>");
        page_content += make_footer();
        server.send(200, FPSTR(TXT_CONTENT_TYPE_TEXT_HTML), page_content);
    }
...
void wifiConfig() {
    ...
    debug_out(wlanssid, DEBUG_MIN_INFO, 1);
    debug_out(F("PPD_read: "), DEBUG_MIN_INFO, 0);
    debug_out(String(ppd_read), DEBUG_MIN_INFO, 1);
    debug_out(F("SDS_read: "), DEBUG_MIN_INFO, 0);
    debug_out(String(sds_read), DEBUG_MIN_INFO, 1);
    debug_out(F("PMS24_read: "), DEBUG_MIN_INFO, 0);
    debug_out(String(pms24_read), DEBUG_MIN_INFO, 1);
    debug_out(F("PMS32_read: "), DEBUG_MIN_INFO, 0);
    debug_out(String(pms32_read), DEBUG_MIN_INFO, 1);
    debug_out(F("HPM_read: "), DEBUG_MIN_INFO, 0);
    ...
    debug_out(F("DS18B20_read: "), DEBUG_MIN_INFO, 0);
    debug_out(String(ds18b20_read), DEBUG_MIN_INFO, 1);
    debug_out(F("HTU21D_read: "), DEBUG_MIN_INFO, 0);
    debug_out(String(htu21d_read), DEBUG_MIN_INFO, 1);
    debug_out(F("BMP_read: "), DEBUG_MIN_INFO, 0);
...
//			"aa" = 170, "ab" = 171, "c0" = 192
            ...
            if (len == 10 && checksum_ok == 1 && (long(act_milli - starttime) > (long(sending_intervall_ms) - long(reading_time_SDS_ms)))) {
                if ((! isnan(pm10_serial)) && (! isnan(pm25_serial))) {
                    ...
                    debug_out(Float2String(float(pm10_serial) / 10), DEBUG_MED_INFO, 1);
                    debug_out(F("PM2.5 (sec.): "), DEBUG_MED_INFO, 0);
                    debug_out(Float2String(float(pm25_serial) / 10), DEBUG_MED_INFO, 1);
                    sds_val_count++;
                }
                len = 0;
                checksum_ok = 0;
...
//			"aa" = 170, "ab" = 171, "c0" = 192
            ...
            if (len == msg_len && checksum_ok == 1 && (long(act_milli - starttime) > (long(sending_intervall_ms) - long(reading_time_SDS_ms)))) {
                if ((! isnan(pm1_serial)) && (! isnan(pm10_serial)) && (! isnan(pm25_serial))) {
                    ...
                    debug_out(Float2String(float(pm1_serial)), DEBUG_MED_INFO, 1);
                    debug_out(F("PM2.5 (sec.): "), DEBUG_MED_INFO, 0);
                    debug_out(Float2String(float(pm25_serial)), DEBUG_MED_INFO, 1);
                    debug_out(F("PM10 (sec.) : "), DEBUG_MED_INFO, 0);
                    debug_out(Float2String(float(pm10_serial)), DEBUG_MED_INFO, 1);
                    pms_val_count++;
                }
                len = 0;
                checksum_ok = 0;
    ...
    if (send_now) {
        ...
        if (pms_val_count > 0) {
            ...
            debug_out("-------", DEBUG_MIN_INFO, 1);
            last_value_PMS_P0 = float(pms_pm1_sum) / (pms_val_count * 1.0);
            last_value_PMS_P1 = float(pms_pm10_sum) / (pms_val_count * 1.0);
            last_value_PMS_P2 = float(pms_pm25_sum) / (pms_val_count * 1.0);
...
//			"aa" = 170, "ab" = 171, "c0" = 192
    ...
    if (send_now) {
        ...
        if (hpm_val_count > 0) {
            debug_out("PM2.5: " + Float2String(float(hpm_pm25_sum) / (hpm_val_count * 1.0)), DEBUG_MIN_INFO, 1);
            debug_out("PM10:  " + Float2String(float(hpm_pm10_sum) / (hpm_val_count * 1.0)), DEBUG_MIN_INFO, 1);
            debug_out("-------", DEBUG_MIN_INFO, 1);
            last_value_HPM_P1 = float(hpm_pm10_sum) / (hpm_val_count * 1.0);
            last_value_HPM_P2 = float(hpm_pm25_sum) / (hpm_val_count * 1.0);
...
void setup() {
    ...
    if (hpm_read) {
        debug_out(F("Lese HPM..."), DEBUG_MIN_INFO, 1);
    }
    if (dht_read) {
