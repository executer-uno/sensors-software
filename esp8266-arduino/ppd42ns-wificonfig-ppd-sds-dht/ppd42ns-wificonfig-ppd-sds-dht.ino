...
void webserver_values() {
	...
	if (WiFi.status() != WL_CONNECTED) {
		...
		page_content += "<tr><td colspan='3'>&nbsp;</td></tr>";
		page_content += table_row_from_value(F(***REMOVED***),FPSTR(INTL_SIGNAL),  String(signal_strength),"dBm");
		page_content += table_row_from_value(F(***REMOVED***),FPSTR(INTL_QUALITAT),String(signal_quality),"%");
		page_content += F("</table>");
		page_content += make_footer();
		server.send(200,FPSTR(TXT_CONTENT_TYPE_TEXT_HTML),page_content);
	}
